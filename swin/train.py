...
from swin.models.swin import Swin, get_opt_param_groups, get_warmup_tuner
...
def get_optimizer(model, total_batch_size, hyp):
    ...
    pg = get_opt_param_groups(model)

    if hyp['optimizer'] == 'adam':  # https://pytorch.org/docs/stable/_modules/torch/optim/lr_scheduler.html#OneCycleLR
        optimizer = optim.Adam(pg, lr=hyp['lr0'], betas=(hyp['momentum'], 0.95), weight_decay=hyp['weight_decay'])  # adjust beta1 to momentum
    elif hyp['optimizer'] == 'adamw':
        optimizer = optim.AdamW(pg, lr=hyp['lr0'], betas=(hyp['momentum'], 0.95), weight_decay=hyp['weight_decay'])
    else:
        optimizer = optim.SGD(pg, lr=hyp['lr0'], momentum=hyp['momentum'], weight_decay=hyp['weight_decay'], nesterov=False)
...
def train(hyp, tb_writer, opt, device):
    ...
    # Create model
    model = Swin(opt.cfg, image_size=opt.img_size[0], nc=nc).to(device)
